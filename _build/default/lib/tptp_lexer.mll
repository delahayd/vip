...
and quoted buf q = parse
  | "\\'"                     { Buffer.add_char buf '\''; quoted buf q lexbuf }
  | "\\\""                    { Buffer.add_char buf '"'; quoted buf q lexbuf }
  | "\\\\"                    { Buffer.add_char buf '\\'; quoted buf q lexbuf }
  | '\'' | '"' as c           { if c = q then QUOTED (Buffer.contents buf) else (Buffer.add_char buf c; quoted buf q lexbuf) }
  | newline                   { Buffer.add_char buf '\n'; Lexing.new_line lexbuf; quoted buf q lexbuf }
  | eof                       { raise (Lexing_error "Chaîne quotée non terminée") }
  | _ as c                    { Buffer.add_char buf c; quoted buf q lexbuf }
